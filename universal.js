document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.main-form[data-universal-form]').forEach(form => {
    ...
    if (decoyLink) {
      decoyLink.addEventListener('click', e => { e.preventDefault(); decoyLinkClicked = true; });
    }
    ...
    if ($.fn.selectpicker) {
      const $countrySelectPicker = $(countrySelect).selectpicker();
      const $stateSelectsPicker = $form.find('#state, #state-2, .states-australia, .states-brazil, .states-canada, .states-china, .states-ireland, .states-india, .states-italy, .states-mexico').selectpicker();

      $countrySelectPicker.on('shown.bs.select', function() {
        const selectpicker = $(this).data('selectpicker');
        if (selectpicker) selectpicker.$menuInner[0].scrollTop = 0;
      });
      $stateSelectsPicker.on('shown.bs.select', function() {
        const selectpicker = $(this).data('selectpicker');
        if (selectpicker) selectpicker.$menuInner[0].scrollTop = 0;
      });
    }
          ...
          fetch('https://faas-nyc1-2ef2e6cc.doserverless.co/api/v1/web/fn-3627560b-2163-4a62-81db-3a3b5da17d5a/ip/info')
            .then(r => r.json()).then(data => {
              ...
              if (countrySelect && data && data.country) {
                ...
                if (opt) { $(countrySelect).val(data.country).selectpicker('refresh').trigger('change'); }
              }
            }).catch(failure);
        }
      });
    ...
    function setStateDropdown(selectedCountry) {
        ...
        form.querySelectorAll(Object.values(map).join(', ')).forEach(dd => { if (dd) dd.style.display = 'none'; });
    ...
    function updateCheckboxErrorClass() {
        ...
        if (isCheckboxInteracted) {
            if (checkbox.checked) {
                ...
                // Добавляем ошибку только если страна не США (где чекбокс не обязателен)
    ...
    $form.validate({
      ...
      errorPlacement: (error, element) => {
        if ($(element).data('interacted')) {
          error.appendTo(element.closest(".field-row, .w-full, .iti"));
        }
      },
      highlight: (element) => {
        if ($(element).data('interacted')) {
      ...
      rules: {
        ...
        'agreement': {
            required: function() {
                return countrySelect && countrySelect.value !== 'United States';
            }
        }
      },
      messages: { /* Сообщения по умолчанию или кастомные */ },
      invalidHandler: function() {
          isCheckboxInteracted = true; // При попытке отправки считаем, что с чекбоксом взаимодействовали
          updateCheckboxErrorClass();
      }
    });
    ...
    form.addEventListener('submit', function(event) {
      ...
      // Помечаем все поля как "interacted", чтобы показать все ошибки
      $form.find('input, select, textarea').data('interacted', true);
      
      if (isSubmitting || !$form.valid()) {
        isCheckboxInteracted = true; // Также для чекбокса
        updateCheckboxErrorClass();
        updateSubmitButtonState();
        return;
      }
      ...
      const dataToSubmit = {
        ...
        'c_of_utm_term': urlParams.get('utm_term') || '', '_ga': getCookieValue('_ga'), 'c_of__ga': getCookieValue('c_of__ga')
      };
      ...
      if (window.dataLayer) {
        window.dataLayer.push({
          ...
          'email': emailInput ? emailInput.value.trim() : '', 'phone': dataToSubmit.full_phone_number,
        });
      }
